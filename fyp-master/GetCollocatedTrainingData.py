...
for y in range (0, bio_data.shape[0]):  #shape[0] = 401 (height)
    for x in range (0, bio_data.shape[1]):
        if mask[y][x] == 1 and bio_data[y][x] >= 0:
            ...
            if bio_data[y][x] >= float(350): #300
                coordsH.append([x, y])
            elif bio_data[y][x] >= float(290): #240
                coordsM.append([x, y]) 
...
for x in range (0, len(coordsM)):
    if not (coordsM[x][0] >= radarW) or (coordsM[x][1] >= radarH):            
        pix = raster_BandRadar.getPixelFloat(int(coordsM[x][0]), int(coordsM[x][1]))
        if (pix > 0):
                pixelData.append(pix)
                labels.append(1)
...
pickle.dump(pixelData, open(savePath + productName + '_' + bandName + '_collocateddata', 'wb'))
pickle.dump(labels, open(savePath + productName + '_' + bandName + '_collocatedlabels', 'wb'))
print("Data saved")
